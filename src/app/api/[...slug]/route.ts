...
async function handle(req: NextRequest, { params }: { params: Promise<{ slug: string[] }> }) {
  ...
  // Application Detail or sub-routes
  if (slugPath.startsWith("applications/")) {
    if (slugPath.endsWith("/requirements")) {
      return NextResponse.json(REQUIREMENTS);
    }
    if (slugPath.endsWith("/sections")) {
      return NextResponse.json(SECTIONS);
    }
    if (slugPath.endsWith("/fields")) {
      return NextResponse.json(FIELDS);
    }
    if (slugPath.endsWith("/progress")) {
      return NextResponse.json({ progress: 15, completed_fields: 3, total_fields: 18, missing_documents: [] });
    }
    if (slugPath.endsWith("/documents")) {
      return NextResponse.json(DOCUMENTS);
    }
    if (slugPath.endsWith("/validation")) {
      return NextResponse.json({ valid: true, messages: [] });
    }
    if (slugPath.endsWith("/imports")) {
    ...
    if (slugPath.endsWith("/clear-fields")) {
      return NextResponse.json({ status: "cleared", field_count: 0 });
    }
    if (slugPath.endsWith("/declaration")) {
  ...
  if (slugPath === "documents" || slugPath.startsWith("documents/")) {
    if (slugPath.endsWith("/photo-analysis")) {
      return NextResponse.json({ pass: true, score: 98, checks: [] });
    }
    if (slugPath.endsWith("/signature-analysis")) {
      return NextResponse.json({ pass: true, score: 95, checks: [] });
    }
    return NextResponse.json(DOCUMENTS);
  }
